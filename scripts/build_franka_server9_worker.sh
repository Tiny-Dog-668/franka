...
output_dir="${repo_root}/dist/franka_server9"
output_path="${output_dir}/franka_server9_streaming_worker"
...
if [[ ! -f "${source_root}/include/franka/robot.h" ]]; then
...
g++ -O3 -DNDEBUG -std=c++17 -pthread \
  ...
  "${repo_root}/native/franka_server9_streaming_worker.cpp" \
  "${libfranka_library}" \
