...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the latest exported_0402 TorchScript policy on Franka."
    )
    parser.add_argument(
        "--config",
...
def main() -> int:
    ...
    config.runner.run_name = f"{config.runner.run_name}_agent_{step}"
    ...
    if args.preview_only:
        print("Running exported_0402 in preview-only mode on the real Franka setup.")
    else:
        print("Running exported_0402 on the real Franka robot.")
